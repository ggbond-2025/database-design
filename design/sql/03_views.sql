...
SELECT
    ...
    COUNT(CASE WHEN e.djx_Status13 IN ('SELECTED', 'COMPLETED') THEN e.djx_EnrollmentId13 END) AS djx_SelectedCount13
...
SELECT
    ...
    SUM(CASE WHEN g.djx_Score13 >= 60 THEN 1 ELSE 0 END) AS djx_PassCount13,
...
SELECT
    r.djx_RegionId13,
    r.djx_RegionName13,
    COUNT(s.djx_StudentId13) AS djx_StudentCount13
FROM Dengjx_Regions13 r
LEFT JOIN Dengjx_Students13 s ON s.djx_RegionId13 = r.djx_RegionId13
GROUP BY r.djx_RegionId13, r.djx_RegionName13;
...
SELECT
    ...
    SUM(CASE WHEN g.djx_Score13 >= 60 THEN 1 ELSE 0 END) AS djx_PassedCourses13,
    s.djx_TotalCredits13,
    COALESCE(SUM(CASE WHEN g.djx_Score13 >= 60 THEN c.djx_Credit13 ELSE 0 END), 0) AS djx_CalculatedCredits13
...
LEFT JOIN Dengjx_Courses13 c ON c.djx_CourseId13 = a.djx_CourseId13
GROUP BY
    s.djx_StudentId13,
    s.djx_Sno13,
    s.djx_Sname13,
