...
async function load() {
  loading.value = true
  try {
    schedule.value = await teacherGet<ScheduleResponse>('/schedule')
  } finally {
    loading.value = false
  }
}
...
<template>
  <PageContainer title="我的课表" description="查询本人任课课表，按上课时间段和周一到周五分别展示。">
        ...
        <template #default="{ row }">
          <div v-if="slotRows(weekday, row).length" class="schedule-list">
            ...
            >
              ...
              <small>{{ item.djx_classname13 }} / {{ formatCourseType(item.djx_coursetype13) }}</small>
